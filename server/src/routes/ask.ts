...
async function buildSystemPromptWithAggregates(userId: string): Promise<string> {
  ...
  return (
    ...
    'Write answers in concise, readable Markdown. Keep paragraphs short and separated by blank lines. ' +
    'For lists, put each bullet on its own line using "- " or numbers like "1. ". ' +
    'Use bold for key ideas (e.g., **Principle**). ' +
    'When a diagram helps, include a position using one of these forms: ' +
    'Inline: "FEN: <FEN>" or fenced: ```fen\n<FEN>\n```; you may include multiple fenced FEN blocks. ' +
    'Only include FEN when it materially clarifies the point. ' +
